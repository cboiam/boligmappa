...
public class GetPopularUsersHandler : Handler.EventHandler
{
    private readonly IUserHandler userHandler;

    public GetPopularUsersHandler(HubConnection connection, IServiceProvider serviceProvider) : base(connection)
    {
        userHandler = serviceProvider.GetRequiredService<IUserHandler>();
    }

    protected override async Task<object> Handle() =>
        await userHandler.GetPopularUsers();
}
