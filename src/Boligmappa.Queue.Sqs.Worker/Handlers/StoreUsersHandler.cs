...
public class StoreUsersHandler : Handler.EventHandler
{
    private readonly IUserHandler userHandler;

    public StoreUsersHandler(HubConnection connection, IServiceProvider serviceProvider) : base(connection)
    {
        userHandler = serviceProvider.GetRequiredService<IUserHandler>();
    }

    protected override async Task<object> Handle() =>
        await userHandler.StoreUsers();
}
